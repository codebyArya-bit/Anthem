...
export function HeroBrandPattern({ className = "", darkTheme = false }: HeroBrandPatternProps) {
  return (
    <div className={`absolute inset-0 pointer-events-none -z-10 select-none overflow-hidden ${className}`}>
      ...
      {darkTheme ? (
        <div className="absolute inset-0 bg-[linear-gradient(to_right,rgba(1,122,202,0.03)_1px,transparent_1px),linear-gradient(to_bottom,rgba(1,122,202,0.03)_1px,transparent_1px)] bg-[size:36px_36px] opacity-100" />
      ) : (
      ...
      {darkTheme ? (
        <div className="absolute inset-0 bg-gradient-to-b from-transparent via-[#003B66]/20 to-[#003B66]" />
      ) : (
